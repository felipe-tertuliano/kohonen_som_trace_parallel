...
void test_3d_classes(double *const *data, int N)
{
    ...
    for (i = 0; i < N; i++)
    {
        int my_class = rand() % num_classes;  // select a random class for the point

        // create random coordinates (x,y,z) around the centre of the class
        data[i][0] = _random(centres[my_class][0] - R, centres[my_class][0] + R);
        data[i][1] = _random(centres[my_class][1] - R, centres[my_class][1] + R);
        data[i][2] = _random(centres[my_class][2] - R, centres[my_class][2] + R);
    }
}
